...
public:
	...
	friend inline bool operator==(const char* str, const Lexer::Token& token) {
		return token == str;
	}
	inline bool operator!=(const char* str) const {
		return strcmp(_data, str) != 0;
	...
	friend inline std::ostream& operator<<(std::ostream& stream, const Token& token) {
		return stream << token._data;
	}
};
// TODO: implement this
typedef std::vector<Token> Tokenized;
