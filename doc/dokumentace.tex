...
\begin{titlepage}
    \centering
    {\Huge \textbf{Compiler vlastního programovacího jazyka}\par}
    \vspace{2cm}
    {\Large Dokumentace k maturitní práci\par}
    \vfill

    \includesvg[width=0.4\textwidth]{logo_gp.svg}
    \vfill
...
\chapter{Úvod}
Jako cíl svého projektu jsem si vymezil tvorbu compileru pro jednoduchý programovací jazyk, protože mě programovací jazyky již delší dobu zajímají. Mít vlastní programovací jazyk je pro mě zajímavé, protože by mi to umožnilo vysokou úroveň kontroly nad tím, co je v jazyce možné. Mám v plánu, že na projektu budu pokračovat i po maturitní zkoušce, takže chci, aby šel projekt snadno rozšiřovat a příliš mi nezáleží na komfortu uživatele jazyka.

To znamená, že mým hlavním cílem je implementace základních jazykových konstrukcí, nikoli plnohodnotného jazyka. Mezi věci, které chci implementovat patří smyčky, podmínky, operátory, proměnné, funkce a další. Můj postup, jak jsem jej naplánoval, spočívá v implementaci jednotlivých částí kompilace v pořadí ve kterém jsou
při běhu compileru potřeba. V tom pořadí jsou také uvedeny v programátorské dokumentaci.
...
Jazyk se stále vyvíjí a informace, které by specifikace obsahovala, by nemusely platit pro novější verze. Uživatelská dokumentace také popisuje
