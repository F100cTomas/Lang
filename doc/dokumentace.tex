...
\begin{abstract}
Práce se zabývá vytvářením jednoduchého compileru programovacího jazyka vytvořeného specificky pro tento projekt za pomoci knihovny LLVM.
...
pracovat po maturitě. Z toho důvodu je celý projekt přizpůsobený mé poněkud netradiční vizi pro syntaxi finálního jazyka. V jazyce tedy není v aktuálním
stavu snadné programovat a mnoho věcí, které by programátor očekával, chybí. To je v souladu s cílem projektu, protože plně rozsáhlý programovací
jazyk by potřeboval množství času mnohem vyšší, než je v rámci maturitního projektu možné.

Projekt jsem naprogramoval v jazyce C++ za použití knihovny LLVM, která je pro compilery populární volbou. Compiler je rozdělen na několik fází, které
překládají původní kód z jedné reprezentace na druhou. Poslední reprezentací je tedy výsledný spustitelný soubor. Zdrojový kód je modulárně strukturovaný,
takže jednotlivé fáze kompilace jsou od sebe oddělené. Jednotlivé fáze jsou podrobně popsané v programátorské dokumentaci. Kromě toho programátorská
dokumentace rozebírá obecnou strukturu projektu. \enquote{Kostra} označuje kód, který nepatří žádné kompilační fázi.

Uživatelská dokumentace je méně podrobná, než programátorská. Slouží spíše jako ukázka programů, které jdou v jazyce psát, než jako specifikace.
Jazyk se stále vyvíjí a informace, které by specifikace obsahovala, by nemuseli platit pro novější verze. Uživatelská dokumentace také popisuje
instalační proces a uživatelské rozhraní.
...
reprezentací kompilovaného kódu. Compiler tedy převede zdrojový kód jazyka do LLVM IR, které pak knihovna LLVM převede na objektový kód cílové platformy.
Jeden z důvodů, proč jsem si projekt vybral je, že jsem se chtěl naučit s LLVM.
\paragraph{\href{https://lld.llvm.org/}{LLD}}
LLD je linker, který je součástí projektu LLVM. Linker vytváří z objektového kódu spustitelné soubory. Můj compiler ho zavolá automaticky.
\paragraph{\href{https://github.com/gouwsxander/easy-args}{Easyargs}}
Knihovnu Easyargs jsem použil hlavně proto, že poskytuje nenáročné rozhraní pro definování argumentů příkazového řádku. Uživatelské rozhraní nepovažuji
...
Clang Format je nástroj, který zajišťuje, že všechen kód v projektu je správně zformátovaný.
\paragraph{\href{https://clang.llvm.org/extra/clang-tidy/}{Clang Tidy}}
Clang Tidy je nástroj, který pomáhá s hledáním chyb v kódu ještě předtím, než je vůbec zkompilován.
\paragraph{\href{https://git-scm.com/}{Git}}
Pro zálohování změn v projektu jsem využil Git a projekt je veřejně viditelný přes repozitář na GitHubu. \cite{lang_repo}
...
podotknout, že to nebyla ideální volba a vedla k častému zamrzání obrazovky a dalším nepříjemnostem.
...
protože jsem ChatGPT používal už před začátkem projektu. Hlavní roli měla AI v debugování kódu. Umělá inteligence má totiž nadlidskou schopnost
hledat hrubé chyby. Dále mi umělá inteligence pomohla pochopit věci, které jsem se v rámci projektu učil nově. AI se hodně podílela na sepsání Makefile.
...
jednoduché rozhraní pro konverzi kódu z jedné reprezentace na druhou. Tyto reprezentace jsou si poměrně blízké, ale svým vrstvením dokážou
...
\begin{itemize}
 ...
 \item Preparser -- Parsuje syntaxi klíčových slov a sestavuje tabulku symbolů.
...
to dokonce některé fáze kompilace jako například Generátor LLVM kódu několikrát celé přepsat, než jsem byl spokojený s výsledkem.
...
úkol. Teprve když byl hlavní soubor hotov, implementoval jsem třídy a metody, které jsem si v hlavním souboru vymyslel. Můj přístup
...
což je vestavěná knihovna C++ pro výpis do konzole. Pokud nejste seznámeni s programováním v jazyce C++, nebuďte překvapeni syntaxí
...
nezpracovaného kódu vytvořit seznam elementárních jednotek syntaxe. Syntax mého jazyka se totiž neskládá výhradně ze znaků, nýbrž často
...
v tom místě nesmí seskupovat znaky. Speciální znaky mají každý svá vlastní pravidla tokenizace. Pokud uživatel formátuje svůj kód rozumně,
neměl by mít s Lexerem v jazyce potíže.
...
syntaxi klíčových slov. Aby se nemusela opakovat práce, uloží si tato fáze syntaxi libovolného klíčového slova ve strukturovaném formátu.
...
operátory jsou operátory, které operují jenom na hodnotě za nimi. Příkladem prefix operátoru je negace ($!x$), nebo předpona minus ($-x$).
Postfix operátory naopak operují na hodnotě před nimi. V programování jsou celkem vzácné. Příkladem by byl post-decrement operátor ($x--$).
...
tedy spolehnout na kód pro řešení klíčových slov, který naopak dokáže popsat téměř jakoukoli syntaxi. Proto jsou závorky klíčovým slovem.
...
Kromě toho komplikovaného procesu si zkušený programátor v kódu výše něčeho všimne. Program nahoře je totiž velmi špatně napsaný. Hned jako první
...
Je patrné, že můj compiler zatím při generaci LLVM IR nedělá vůbec žádnou optimalizaci. Nedělá totiž optimalizaci nikde. To je v souladu
s mými cíli, protože by implementovat pořádné optimalizace ubíralo čas od implementace mnohem důležitějších věcí.
...
pak zavolá linker, (já si vybral LLD) který pak ze souboru s objektovým kódem udělá spustitelný soubor. Kromě toho nemá smysl zacházet tady do větších detailů, protože jsem velkou část zkopíroval z tutoriálu na stránkách LLVM\cite{llvm_tutorial}, takže pokud vás nejen tato část projektu zaujala, zkuste si ho přečíst.
...
Ze začátku jsem se pokoušel zprovoznit kompilaci přes Visual Studio\footnote{Mluvím zde o programu Visual Studio, nikoli Visual Studio Code. Visual Studio Code
nezahrnuje C++ SDK.}, kanonický nástroj pro vytváření Windows native aplikací. Prvním problémem bylo, že nemohu prostě vložit svůj Makefile do
Visual Studia a očekávat, že to bude fungovat. Visual Studio podporuje tři build systémy pro C++: MSBuild, CMake a NMake. Jeden z nich jsem simusel vybrat.
MSbuild jsem ihned zamítl, protože funguje na základě složitě navigovatelných menu, takže to bylo mezi CMake a NMake. S CMake mám předchozí zkušenost,
tedy hlavně negativní předchozí zkušenost, zatímco NMake je port Makefile pro Windows. Bohužel ale Makefile a NMake jsou dostatečně odlišné,
že bych stejněmusel mít soubory na kompilaci dva. Protože jsem neměl náladu ani energii, rozhodl jsem se pro CMake. Myslel jsem si, že umělá inteligence,
která je trénovaná na mnohem populárnějším CMake mi dá, alespoň v prvních pár pokusech, alespoň částečně funkční build skript. Ukázalo se ale,
že Visual Studio nemá žádný bezbolestný způsob, jak si stáhnout knihovnu LLVM, takže jsem si ji musel stáhnout manuálně, nebo si ji dokonce zkompilovat sám.
Nemyslím si, že projekt rozměru mého má právo žádat o uživateli zkompilovat knihovnu velikosti LLVM, takže jsem si začal stahovat LLVM,
když jsem dostal nápad, čímž tento pokus oficiálně skončil.
...
Samozřejmě to nebylo mým jedinným důvodem. Celý tento proces měl mnoho nevýhod.
...
přesunout je do stejné složky, což není něco, co by měl člověk dělat.

Přesto to byl pravděpodobně ten správný přístup. Díky tomu jsem nemusel udržovat dvě verze kompilačních skriptů v jiných jazycích
a byl jsem schopný dát vše potřebné ke kompilaci projektu na jeden přenosný flash disk. Můžu tedy projekt upravovat a kompilovat na
libovolném počítači i bez administrátorských práv.
...
Aktuálně projekt podporuje dvě platformy: Linux a Windows. Windows verze funguje díky vrstvě kompatibility MinGW, takže bez ní nelze spustit.
Projekt momentálně není ve zkompilované formě dostupný, takže je nutné si jej zkompilovat sám.
...
\chapter{Závěr}
S výsledkem svého projektu jsem spokojen. Všech hlavních cílů se mi podařilo včas dosáhnout. Naopak s vedlejšími cíli jsem byl příliš ambiciózní, což jsem i
...
jazyk stále velmi daleko, je OOP. Nejraději bych, aby se jazyk dostal tak daleko, že mohu jeho compiler přepsat ve stejném jazyce, ale to v momentální
době ještě nevypadá realisticky. Tyto cíle však spadají do daleké budoucnosti a krátkodobě se budu zaměřovat spíše na zvládnutí toho, co jsem určil jako
vedlejší cíle a ještě není hotové.
...
\bibitem{lang_repo}
Ondřej Suk.
\textit{GitHub repozitář projektu}.
\url{https://github.com/F100cTomas/Lang}

\bibitem{llvm_tutorial}
LLVM Project.
\textit{My First Language Frontend with LLVM Tutorial}.
