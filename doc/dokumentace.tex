...
Makefile je systém, pomocí kterého jsem vytvořil kompilační script pro tento projekt. Vybral jsem si ho, protože na rozdíl od jiných C++ build systémů
...
Existuje několik tabulek zároveň. Jednotlivé tabulky mají mezi sebou hiearchickou strukturu. Hlavní tabulka popisuje globálně viditelné
symboly, tedy globální proměnné a funkce, zatímco podřízené tabulky popisuje lokální symboly.
...
se jedná o složitější operátor, jako jsou například závorky, nedokážeme jej pomocí těchto čtyřech druhů samotných popsat. Musíme se
...
pak zavolá linker, (já si vybral LLD) který pak ze souboru s objektovým kódem udělá spustitelný soubor. Má asi největší poměr platformově závislého
kódu v celém projektu, protože jsem měl tady s Windows největší potíže. Kromě toho nemá smysl zacházet tady do větších detailů, protože jsem
velkou část zkopíroval z tutoriálu na stránkách LLVM: \href{https://llvm.org/docs/tutorial/MyFirstLanguageFrontend/index.html}{https://llvm.org/docs/tutorial/MyFirstLanguageFrontend/index.html}

\section{Problémy s Windows}
Co jsem u projektu nečekal, bylo, jak příšerně je Windows k programování compilerů navržen. Jelikož nemám trvalý přístup k hardwaru, na kterém
běží Windows, musel jsem většinu portování dělat na svém často zamrzajícím VM, což upřímně trochu zkazilo mojí naladu z projektu a ubralo mi
cenný čas. Problémy, které však budu popisovat jsou univerzální všem Windows počítačům. I kdyby byl Windows má primární platforma, jediné,
co by to znamenalo by bylo, že bych měl méně času, protože bych se musel věnovat spravování Windows-specifických problémů déle.
...
MSBuild, CMake a NMake. MSbuild jsem ihned zamítl, protože funguje na základě složitě navigovatelných menu, takže to bylo mezi CMake a NMake.
S CMake mám předchozí zkušenost, tedy hlavně negativní předchozí zkušenost, zatímco NMake je port Makefile pro Windows. Bohužel ale Makefile
a NMake jsou dostatečně odlišné, že bych stejně musel mít soubory na kompilaci dva. Protože jsem neměl náladu ani energii, rozhodl jsem se
pro CMake. Myslel jsem si, že umělá inteligence, která je trénovaná na mnohem populárnějším CMake mi dá, alespoň v prvních pár pokusech, alespoň
částečně funkční build skript. Ukázalo se ale, že Visual Studio nemá žádný bezbolestný způsob, jak si stáhnout knihovnu LLVM, takže jsem si
ji musel stáhnout manuálně, nebo si ji dokonce zkompilovat sám. Nemyslím si, že projekt rozměru mého má právo žádat po uživateli zkompilovat
knihovnu velikosti LLVM, takže jsem si začal stahovat LLVM, když jsem dostal nápad, čímž tento pokus oficiálně skončil.

\subsection{Druhý pokus -- Terminál}
Můj nápad byl jednoduchý: \enquote{Jestli si stejně musím stahovat celé LLVM, co mi tady poskytuje Visual Studio?} Uvědomil jsem si totiž,
...
přeskočit a rovnou spouštět compiler pomocí terminálu. Co mi poskytuje build systém, který mě stejně odpuzuje, když kompilační script můžu napsat
v jakémkoli skriptovacím jazyce schopném spouštět příkazy přes příkazový řádek. Takže jsem nechal ChatGPT mi vygenerovat BATCH script, který
...
native aplikací není pouze editorem. Je to správce celé řady softwaru, který je potřeba při vývoji softwaru. Pokud bylo Visual Studio pryč
...
Jsem poněkud zklamán, že mi čas nedovolil implementovat některé mé vedlejší cíle, ale zato jsem rád, že jsem nebyl s výjimkou dokumentace v mém
